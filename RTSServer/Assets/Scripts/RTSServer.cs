...
public class RTSServer : MonoBehaviour
{
    public static RTSServer instance;
    ...
    Dictionary<float, string> statRPCs = new Dictionary<float, string>();//I know not to declare variables this far down
    public void UpdateStatRPC(int aNetId, int aType, float aValue)
    {
        //most of this is to prevent the echo effect from one person's callback RPC
        //from creating an infinite loop where the setvalue that is called from a stat change
        //triggers a new callback for the exact same statchange
        string recentRPC = aNetId + " " + aType + " " + aValue;
        bool echo = false;
        foreach (var item in statRPCs)
        {
            if (item.Value == recentRPC 
                && Time.time - item.Key < 2)
            {
                echo = true;
                statRPCs.Remove(item.Key);
            }
        }
        statRPCs[Time.time] = recentRPC;
        if (!echo)
        {
            serverNet.CallRPC("SetValue", UCNetwork.MessageReceiver.AllClients, aNetId, aType, aValue);
        }
    }
}
